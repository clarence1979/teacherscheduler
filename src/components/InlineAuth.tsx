...
const InlineAuth: React.FC<InlineAuthProps> = ({ onAuthSuccess, initialError }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isSupabaseAvailable()) {
      setError('Database connection not available. Please configure your Supabase URL and API key in the .env file.');
      setLoading(false);
      return;
    }
    ...
    <div>
      <div className="mb-6 text-center">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">
          {isSignUp ? 'Get started with your teacher account' : 'Welcome back, Teacher'}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-blue-600 text-white py-3 px-4 rounded-lg font-medium hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-all duration-200"
        >
          {loading ? (
        ...
        {(!googleAuth.isConfigured() && !microsoftAuth.isConfigured()) && (
          <div className="text-xs text-center text-yellow-600 dark:text-yellow-400 mt-2">
            SSO not configured. Contact administrator.
          </div>
        )}
